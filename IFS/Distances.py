...
def distance(distanceMeasure , A , B, type = None, p = 1 , w = None ): #,k = None , x = None , y = None):
    if distanceMeasure == 'defFuzzyDistance':
        return IFS_Dist00(A , B ,type)
    elif distanceMeasure == 'defIFSDistance':
        return IFS_Dist01(A,B,type)
    elif distanceMeasure == 'wangXinDistance':
        return IFS_Dist02(A , B , type , w , p)
    elif distanceMeasure == 'yangChiclanaDistance':
        return IFS_Dist03(A,B,type)
    elif distanceMeasure == 'grzegorDistance':
        return IFS_Dist04(A , B , type)
    elif distanceMeasure == 'vlachSergDistance':
        return IFS_Dist05(A , B)
    else:
        print('Unknown distance measure input.')
        return None

########################################################################################################################

def IFS_Dist00(A , B , type):
    '''
    ...
    if type == 'H' or type == 'h':
        ...
        print('Distance type parameter must be H, E, nH or nE.')
        return

########################################################################################################################

def IFS_Dist01(A , B , type):
    '''
...
def IFS_Dist02(A , B , type , w  , p = 1):
    ...
    if type == 2 or (type == 1 and (w.size > 1 and w.sum() != 0)):
        if w.size != A.size:
            print('Weight parameter dimensions must be 2-D Nx1 matrix.')
            return None
