...
def distanceThreshold(I , dist , w = np.array([]) , lamdbda = 0.2 , mask = np.array([]) , ty = 'H' , p = 1):
    I = np.array(I)
    x = len(I)
    y = len(I[0])
    if mask.size == 0:
    ...
    for t in range(-1, L - 1):  # L - 1):
        ...
        distances[t+1] = Distances.distance(dist, A, B, w=w , type=ty , p=p)
...
def similarityThreshold(I , sim , w = np.array([]) , lamdbda = 0.2 , mask = np.array([]) , ty = 'H' , p = 1 , omegas = [0.5 , 0.3 , 0.2], a = 1, b = 0, c = 0, lamda = 1):
    I = np.array(I)
    x = len(I)
    y = len(I[0])
    if mask.size == 0:
    ...
    for t in range(-1, L - 1):  # L - 1):
        ...
        similarities[t+1] = Similarities.similarity(sim, A, B, w=w, type=ty , p = p , omegas = omegas , a = a , b = b , c = c)
...
def miscThreshold(I, mis, k=0.5, lamdbda=0.2, mask=np.array([])):
    ...
    for t in range(-1, L - 1):  # L - 1):
        ...
        misc[t + 1] = Miscellaneous.miscs(mis,A,B,k,x,y)
